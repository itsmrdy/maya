...
///
class MayaDatabaseSeeder extends ADatabase implements IDatabase {
  ...
  Future<Either<AppFailure, String>> createTableForDatabase() async {
    ...
    try {
      ...
      if (_database != null) {
        ...
        _database!.execute(
            '''DROP TABLE IF EXISTS User; CREATE TABLE User (id INTEGER PRIMARY KEY, username TEXT, password TEXT, balance REAL, userid INTEGER)''');
      ...
      for (var seeder in seedersList!) {
        if (seeder is UserSeeder) {
          ...
          for (var i = 0; i < userSeeder.data.length; i++) {
            ...
            _database!.execute(
              'INSERT INTO User(username, password, balance, userid) VALUES("${user.username}", "${user.password}", ${user.balance}, "${user.userid}")',
            );
          }
